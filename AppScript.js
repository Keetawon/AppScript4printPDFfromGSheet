  ...
  for (let i = 1; i < data.length; i++) {
    ...
    for (let j = 1; j <= quantity; j++) {
      ...
      const newDocId = fillTemplateWithData(
        docTemplateId,
        {
          customerName, telNo, so_No, po_No,
          project, floor, unitType, unitNo, projectnunitNo, building, houseNo,
          deliveryDate, deliveryTimeRange,
          installedProduct, category, brand, productHi1, productColor, installedRoom, installedPoint, note,
          quantity, index: j
        }
      );
...
function fillTemplateWithData(templateId, data) {
  ...
  body.replaceText('{{telNo}}', data.telNo);
  ...
  body.replaceText('{{Note}}', data.note);
